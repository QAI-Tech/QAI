...
export async function POST(req: NextRequest) {
  try {
    const { description, edge_id } = await req.json();
    ...
    const response = await fetch(constructUrl("FormatEdgeDescription"), {
      method: "POST",
      headers: {
    ...
    console.log("Edge description formatting result", result);
    ...
    return NextResponse.json(
      {
        error: `Failed to format edge description: ${(error as Error).message || error}`,
      },
