...
export async function GET(request: NextRequest) {
  try {
    ...
    // Validate product_id
    if (!request_id) {
      Sentry.captureMessage("RequestId is required", {
        level: "fatal",
        tags: { priority: "high" },
      });
    ...
    const backendResponse = await fetch(
      constructUrl(`GetTestCasesForRequest?request_id=${request_id}`),
      {
        method: "GET",
        headers: {
    ...
    const testCases = await backendResponse.json();

    // Check if test cases are found
    if (!testCases || !testCases.test_cases) {
      return NextResponse.json([], { status: HTTP_STATUS_OK });
    }

    // Return the test cases
    return NextResponse.json(testCases, { status: HTTP_STATUS_OK });
  } catch (error) {
    console.error("Error in GET /api/get_test_cases_for_request:", error);
    Sentry.captureException(error, {
      level: "fatal",
      tags: { priority: "high" },
    });
    return NextResponse.json(
      { error: "Failed to get test cases for product" },
