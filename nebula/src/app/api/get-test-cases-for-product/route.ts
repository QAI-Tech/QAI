...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(
      constructUrl(`GetTestCasesForProduct?product_id=${productId}`),
      {
        method: "GET",
        headers: {
    ...
    if (!testCases || !testCases.test_cases) {
      return NextResponse.json(
        { error: "No test cases found for this product ID." },
    ...
    console.error("Error in GET /api/get_test_cases_for_product:", error);
