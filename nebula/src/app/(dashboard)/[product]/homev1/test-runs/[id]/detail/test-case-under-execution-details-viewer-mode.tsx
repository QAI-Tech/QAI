...
import { ChevronLeft, ChevronRight, Monitor, Play } from "lucide-react";
import type { testCaseSchema, TestCaseUnderExecutionSchema } from "@/lib/types";
import { Criticality, TestCaseStepStatus } from "@/lib/types";
import { GCS_BUCKET_URL } from "@/lib/constants";
...
import { TestCaseUnderExecutionStatus, CommentType } from "@/lib/types";
...
interface TestCaseUnderExecutionDetailsViewerModeProps {
  onClose: () => void;
  onNextTestCase?: () => void;
  onPrevTestCase?: () => void;
  hasNext?: boolean;
...
export function TestCaseUnderExecutionDetailsViewerMode({
  onClose,
  onNextTestCase = () => {},
  onPrevTestCase = () => {},
  hasNext = false,
...
}: TestCaseUnderExecutionDetailsViewerModeProps) {
  ...
  const [mode, setMode] = useState<"flow" | "video">("video");
  ...
  const handleSaveTestCase = async (updateData: {
    ...
    if (!testCaseUnderExecutionId) {
      toast.error("Test case under execution information is missing.");
      return false;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to save comments. Please try again.",
        );
      }
  ...
  const buildLocalTestCaseFromTcue = useCallback(
    (
      ...
      const result: testCaseSchema = {
        ...
        metadata:
          typeof tcue?.metadata === "object"
            ? JSON.stringify(tcue.metadata)
            : (tcue?.metadata as string) || "",
            ...
            <div className="glass-effect rounded-full p-1 mb-4 mt-2">
              <div className="flex gap-1">
                ...
                <Button
                  ...
                  className={`rounded-full px-6 ${mode === "video" ? "bg-purple-600 hover:bg-purple-700" : "text-white hover:bg-white/20"} ${!executionVideoUrl ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                      ...
                      <div className="w-80 h-[700px] flex items-center justify-center bg-gray-100">
                        {isLoading || showSrc === undefined ? (
                          ...
                          <img
                            ...
                            className="w-80 h-[700px] object-contain bg-white"
                          />
                        ) : (
                          <span className="text-gray-400 text-sm">
                            No image available
              ...
              <ScrollArea className="h-full">
                <div className="px-6 py-6 space-y-6 pb-6">
                  ...
                  <CommentsSection
                    comments={comments}
                    isLoading={{
                      status: isSavingComments,
                      action: isSavingComments ? "saving" : null,
                    }}
