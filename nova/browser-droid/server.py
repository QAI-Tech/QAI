...
from app.processors.session_interactions_processor import SessionInteractionsProcessor

from app.routes.device_routes import create_device_blueprint
from app.routes.nova_routes import create_nova_routes
...
def create_app():
    ...
    graph_service = GraphService(config, streaming_service, llm_wrapper)
    session_processor = SessionInteractionsProcessor(llm_wrapper, graph_service)
    ...
    # Set the interval service in annotation service
    ...
    app.register_blueprint(
        create_device_blueprint(adb_service, recording_service, config)
    )
    ...
    app.register_blueprint(create_nova_routes(streaming_service))
    app.register_blueprint(create_graph_blueprint(graph_service))

    app.register_blueprint(
        create_orchestrator_blueprint(
            session_orchestrator, session_processor, llm_wrapper
        )
    )
