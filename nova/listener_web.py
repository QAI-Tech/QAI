...
class SyncPubSubListenerWeb:
    def __init__(self, project_id, subscription_name, max_messages=1):
        """
        Initialize the synchronous Pub/Sub listener for Web executions
        ...
        logger.info(f"Initialized WEB listener for subscription: {self.subscription_path}")
    
    def process_message(self, message):
        """
        Process a single message for Web executions
        ...
        try:
            ...
            logger.info(f"Processing WEB message ID: {message_id}")
            ...
            logger.info(f"Successfully processed WEB message ID: {message_id}")
            return True
            
        except Exception as e:
            logger.error(f"Error processing WEB message {message.message_id}: {str(e)}")
            return False
    ...
    def listen_for_messages(self):
        ...
        """
        logger.info("Starting WEB message listener...")
        ...
        while True:
            try:
                ...
                if not response.received_messages:
                    logger.info("No WEB messages received, continuing to poll...")
                    continue
                ...
                for received_message in response.received_messages:
                    ...
                    logger.info(f"Received WEB message: {message.message_id}")
                    ...
                    try:
                        ...
                        logger.info(f"Acknowledged WEB message: {message.message_id}")
                    except Exception as ack_error:
                        logger.error(f"Failed to acknowledge WEB message {message.message_id}: {ack_error}")
                        continue
                    ...
                    if success:
                        logger.info(f"WEB message {message.message_id} processed successfully")
                    else:
                        logger.error(f"WEB message {message.message_id} processing failed")
                ...
                logger.info("WEB pull request timed out, continuing...")
                continue
                
            except Exception as e:
                logger.error(f"Error in WEB message listener: {str(e)}")
    ...
    def stop_listener(self):
        ...
        """
        logger.info("Stopping WEB message listener...")
        self.subscriber.close()

def main():
    # Configuration for Web executions
    PROJECT_ID = "qai-tech-staging"
    SUBSCRIPTION_NAME = "nova-web-execution-queue-sub"
    
    # Create and start the listener
    listener = SyncPubSubListenerWeb(PROJECT_ID, SUBSCRIPTION_NAME)
