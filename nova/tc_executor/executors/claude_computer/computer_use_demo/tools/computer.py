...
class ComputerTool(BaseAnthropicTool):
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        print(
            f"""### Performing action: {action}{f", text: {text}" if text else ''}{f", coordinates: {coordinate}" if coordinate else ''}"""
        )
    ...
    async def screenshot(self):
        pause=1
        print(f'Pausing for {pause} seconds and afterwards, computer will take the screenshot...')
        await asyncio.sleep(pause)
        """Take a screenshot of the current screen and return the base64 encoded image."""
        # Capture screenshot using PyAutoGUI
        screenshot = await asyncio.to_thread(pyautogui.screenshot)
        print(f"---------------- Screenshot taken: {type(screenshot)} -----------------")
        
        # blanking out the right half part of the screenshot
        width, height = screenshot.size
        white_color = (255, 255, 255)
        white_area = Image.new("RGB", (width//2, height), white_color)
        screenshot.paste(white_area, (width//2, 0))
    ...
    def scale_coordinates(self, source: ScalingSource, x: int, y: int):
        ...
        if source == ScalingSource.API:
            ...
            print(f'-- scaled coords - ass -> real - {round(x * x_scaling_factor)}, {round(y * y_scaling_factor)}')
            return round(x * x_scaling_factor), round(y * y_scaling_factor)
        else:
            # Real screen coordinates -> assistant's coordinate system
            print(f'-- scaled coords - real -> ass - {round(x / x_scaling_factor)}, {round(y / y_scaling_factor)}')
            return round(x / x_scaling_factor), round(y / y_scaling_factor)
