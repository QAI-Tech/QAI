...
def _transcribe_interactions_from_video(video_url):
...
def extract_test_cases_from_video(video_url):
    outdirpath = os.path.join('logs', video_url.split('/')[-1].split('.')[0])
    os.makedirs(outdirpath, exist_ok=True)
    transcribed_interactions, idt = _transcribe_interactions_from_video(video_url)
...
if __name__ == '__main__':
    ...
    extract_test_cases_from_video(video_url)
