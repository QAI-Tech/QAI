...
class LLMWrapper:
    ...
    def __init__(self, api_key: str = None, model_name: str = "gemini-2.5-flash"):
        self.api_key = api_key or os.getenv("GEMINI_API_KEY_STAGING")
    ...
    def describe_transition_action(
        ...
        """Generate a concise description for a transition between two screenshots"""
        before_file = None
        after_file = None
        try:
            ...
            cleaned_response = self._clean_llm_response_text(response_text)
            logger.info(f"Cleaned LLM response: {cleaned_response[:500]}")

            try:
                parsed = json.loads(cleaned_response)
                # Look for formatted_description (new format) or description (old format)
                description = parsed.get("formatted_description", "").strip()
                if not description:
                    # Fallback to old field name for backwards compatibility
                    description = parsed.get("description", "").strip()
                if not description:
                    logger.warning("LLM returned empty formatted_description, using fallback")
                    description = "User interaction"
                else:
                    logger.info(f"Extracted formatted_description: {description}")
            except json.JSONDecodeError as json_err:
                logger.warning(f"Failed to parse JSON: {json_err}, using raw text")
            ...
            logger.error(f"Edge description generation failed: {e}")
            raise Exception(f"Failed to describe transition: {e}")
        finally:
            for uploaded in [before_file, after_file]:
                if uploaded:
                    try:
                        genai.delete_file(uploaded.name)
                        logger.info(f"File {uploaded.name} deleted.")
                    except Exception as cleanup_error:
                        logger.warning(
                            f"Failed to delete file {getattr(uploaded, 'name', 'unknown')}: {cleanup_error}"
                        )

    def _clean_llm_response_text(self, text: str) -> str:
        """Clean LLM response text by removing markdown code block syntax and normalizing quotes"""
        text = text.strip()
    ...
    def _extract_description_fallback(self, text: str) -> str:
        ...
        # Fallback: Try to find "description": "..." pattern (old format)
        match = re.search(r'"description"\s*:\s*"([^"]*)"', text)
        if match:
            return match.group(1).strip()

        # Try to find 'description': '...' pattern (single quotes)
        match = re.search(r"'description'\s*:\s*'([^']*)'", text)
        if match:
            return match.group(1).strip()

        # If text looks like a plain description (no JSON), use it
        clean_text = text.strip()
        if clean_text and not clean_text.startswith('{'):
            # Limit to first 100 chars as fallback
